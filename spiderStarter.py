...
import datetime
from general import (create_storage_dir, create_data_files, file_to_set,
                     set_to_file)
from checks import verifyPublicationDate, verifyLanguage, verifyArticleContent
...
class Spider:
    ...
    def __init__(self, projectNames, baseUrls, domainNames,
                 articleTitle, articleDate, articleBody, articleLang):
    ...
    @staticmethod
    def boot():
        [create_storage_dir(prnames)
         for prnames in Spider.starters["projectNames"]]
        [create_data_files(project, Spider.starters["baseUrls"][k])
         for k, project in enumerate(Spider.starters["projectNames"])]
    ...
    def crawl_page(threadName, pageUrl):
        ...
        try:
            if Spider.crawledDict[projectDetails[1]] is None or \
                    pageUrl not in Spider.crawledDict[projectDetails[1]] or \
                    pageUrl in Spider.starters["baseUrls"]:
                Spider.addLinksToQueue(Spider.gatherLinks(
                    projectDetails, pageUrl, threadName),
                    projectDetails[1], pageUrl, threadName)
    ...
    def gatherLinks(projectDetails, pageUrl, threadName):
        ...
        try:
            ...
            if 'text/html' in response.headers.get('content-Type'):
                ...
                tag = createNone(
                    Spider.starters['articleDate'][currentIndex]['tag'])
                classing = createNone(
                    Spider.starters["articleDate"][currentIndex]['class'])
                meta = createNone(
                    Spider.starters["articleDate"][currentIndex]['meta'])
                tagLang = createNone(
                    Spider.starters['articleLang'][currentIndex]['tag'])
                classingLang = createNone(
                    Spider.starters["articleLang"][currentIndex]['class'])
                metaLang = createNone(
                    Spider.starters["articleLang"][currentIndex]['meta'])
                classingText = createNone(
                    Spider.starters['articleBody'][currentIndex]['class'])
                soup = BeautifulSoup(htmlBytes)
                verifiedDate = verifyPublicationDate(soup, tag, classing, meta)
                verifiedLang = verifyLanguage(
                    soup, tagLang, classingLang, metaLang)
                if (verifiedDate is not False) or \
                        (Spider.pickBaseLink(currentIndex) == pageUrl):
                    print("++++detected++++", verifiedDate)
                    if verifiedDate is not False:
                        dateDifference = datetime.datetime.now().date() - \
                            verifiedDate.date()
                        days = dateDifference.days
                        print("~~~~", days, "day(s)")
                        if days <= 1 and verifyArticleContent(
                            soup, classingText) and verifiedLang and \
                                len(Spider.queueDict[currentName]) < 100:
                            print(":::>>day", threadName, currentName, pageUrl)
                            Spider.saveTheHtml(
                                htmlString, currentName, pageUrl, threadName)
    ...
    def addLinksToQueue(links, name, pageUrl, threadName):
        try:
            for url in links:
                if (url in Spider.queueDict[name]) or \
                        (url in Spider.crawledDict[name]):
                    continue
