...
from testIndexer import Searcher, ShelveIndexes
from langProc import toQueryTerms
import logging
import cgi
from datetime import datetime

searcher = Searcher("rt/", ShelveIndexes)
app = Flask(__name__)
...
def search_results(query, page):
    ...
    query_terms = toQueryTerms(query)
    ...
    docs = []
    for docid in docids:
        docs.append((searcher.indexes.get_url(docid),
                     searcher.generate_snippet(query_terms, docid),
                     searcher.indexes.get_title(docid)))
