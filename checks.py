...
#html_doc = "http://www.newvision.co.ug/new_vision/news/1414057/gov-set-special-development-program-busoga-region"

#html = v2.extract(html_doc)
...
def checkAurthor(passedSoup):
    ...
    aurthor = re.sub('(\t|\n|\[|\])', '', str(BeautifulSoup(str(aurthor)).get_text().encode('utf-8')))
    ...
    try:
        ...
        if (classing is None) and (meta is None):
            ...
            try:
                ...
                d = parser.parse(passedSoup.find(class_ = classing).string, fuzzy = True)
                return d
            except Exception as e:
                try:
                    # testing the class with inner tags
                    if passedSoup.find(class_ = classing):
                        d = parser.parse(passedSoup.find(class_ = classing).get_text(), fuzzy = True)
                        return d
            ...
            try:
                ...
                d = parser.parse(passedSoup.find("meta", property=meta)["content"], fuzzy=True)
                return d
            except Exception as e:
                try:
                    d = parser.parse(passedSoup.find(attrs={"itemprop":meta})["content"], fuzzy=True)
                    return d
                except Exception as e:
                    print e
                    pass
...
def getArticleText(passedSoup, classing, locDict=None):
    if locDict is None:
        return removeOtherTags(removeFromArticleText(passedSoup.find(class_ = classing).get_text()))+[checkTitle(passedSoup)]
    else:
        return removeOtherTags(removeFromArticleText(passedSoup.find(class_ = classing), locDict))+[checkTitle(passedSoup)]
...
def getNewsDate(passedSoup):
    publicationDateString = publicationDate = re.sub('(\t|\n|\[|\])', '',str(BeautifulSoup(str(checkDateValues(passedSoup))).get_text().encode('utf-8')))
    publicationDate = datefinder.find_dates(str(publicationDate.encode('utf-8')))
    for dates in publicationDate:
        if dates is not None:
            extractedDate = re.sub('[\s|\t|\r|\n|\-|:|\/]', ',', str(dates)).split(",")
