...
import shelve
import workaround
from general import readSourceFile
from bs4 import BeautifulSoup
from checks import (verifyLanguage, verifyPublicationDate, getArticleText,
...
class ShelveIndexes(object):
    ...
    def load_from_disk(self, index_dir):
        self.invertedIndex = shelve.open(os.path.join(
            index_dir, "invertedIndex"))
        self.forwardIndex = shelve.open(os.path.join(
            index_dir, "forwardIndex"))
        self.urlToId = shelve.open(
            os.path.join(index_dir, "urlToId"))
    ...
    def startIndexing(self, indexDir):
        ...
        self.forwardIndex = shelve.open(os.path.join(
            indexDir, "forwardIndex"), "n", writeback=True)
        self.urlToId = shelve.open(os.path.join(
            indexDir, "urlToId"), "n", writeback=True)
        self.indexDir = indexDir

    def _mergeBlocks(self):
        print("Merging blocks!")
        blocks = [shelve.open(os.path.join(
            self.indexDir, "invertedIndexBlock{}".format(i)))
            for i in range(self.blockCount)]
        keys = set()
        for block in blocks:
            keys |= set(block.keys())
        print("Total word count", len(keys))
        mergedIndex = shelve.open(os.path.join(
            self.indexDir, "invertedIndex"), "n", writeback=True)
        keyInd = 0
        for key in keys:
            keyInd += 1
    ...
    def _createNewIIBlock(self):
        ...
        if self.invertedIndex:
            self.invertedIndex.close()
        self.invertedIndex = shelve.open(os.path.join(
            self.indexDir, "invertedIndexBlock{}".format(
                self.blockCount)), "n", writeback=True)
        self.blockCount += 1
    ...
    def addDocument(self, threadName, name, url, doc):
        ...
        if url not in self.urlToId:
            ...
            for position, term in enumerate(doc.parsed_text):
                ...
                self.invertedIndex[stem].append(
                    workaround.InvertedIndexHit(currentId, position, doc.score))
            return True
        return False

    def get_documents(self, query_term):
        print("get_document******", self.invertedIndex,
              query_term.stem.encode('utf-8'))
...
class SerpPagination(object):
    def __init__(self, page, page_size, total_doc_num):
        ...
        self.pages = (total_doc_num / page_size) \
            if total_doc_num % page_size == 0 \
            else (total_doc_num / page_size) + 1
...
class SearchResults(object):
    def __init__(self, docs_with_relevance):
        self.docids, self.relevances = zip(*docs_with_relevance) \
            if docs_with_relevance else ([], [])
...
class Searcher(object):
    ...
    def generate_snippet(self, query_terms, doc_id):
        ...
        for pos, term in enumerate(self.indexes.get_document_text(doc_id)):
            if term in query_terms:
                print("&&&&&&&&generate_snippet", term, query_terms)
                query_terms_in_window.append((term, pos))
                if len(query_terms_in_window) > 1 and \
                        query_terms_in_window[0][0] == term:
                ...
                if tiw > terms_in_best_window or \
                        (tiw == terms_in_best_window and
        ...
        snippet = [(term.fullWord, term in query_terms)
                   for term in self.indexes.get_document_text(
                       doc_id)[snippet_start:snippet_end]]
        print("snippet###################", snippet)

        if len(snippet) > 50:
            excessive_len = len(snippet) - 50
            snippet = snippet[:len(snippet)/2 - excessive_len/2] + \
                [("...", False)] + snippet[len(snippet)/2 + excessive_len/2:]
        print("snippet>>>>>>>>>>>>>>>>", snippet)
        return snippet
    ...
    def find_documents_and_rank_by_points(self, query_terms):
        ...
        return SearchResults(sorted(list(
            docids_and_relevance), key=lambda x: x[1], reverse=True))
...
class Preper():
    ...
    def __init__(self, projectNames, baseUrls, articleTitle,
                 articleDate, articleBody, articleLang, articleAvoid):
    ...
    def parseDocument(self, threadName, prname, openFile):
        return self.verifyDocument(
            threadName, self.getCurrentIndex(prname)[0], openFile)

    def getCurrentIndex(self, prname):
        key = [k for k, i in enumerate(
            Preper.busket['projectNames']) if i == prname]
        return key

    def verifyDocument(self, threadName, index, openFile):
        print("verify++++>>>", threadName, index, openFile)
        ...
        # TODO for efficiency, pick all at once
        ...
        if verifiedDate is not False and verifiedLang and verifiedText:
            ...
            if days:
                return getArticleText(soup, classingText,
                                      Preper.busket["articleAvoid"][index]) + \
                    [verifiedDate]
...
def createIndexFromDir(indexesImplementation=ShelveIndexes):
    ...
    baseUrls, articleTitle, articleDate, articleBody, lang, avoid = zip(
        *[(v['link']['siteUrl'], v['title'], v['date'], v['articleBody'],
           v['language'], v['avoid']) for k, v in contents.iteritems()])
    projectNames = contents.keys()
    prep = Preper(projectNames, list(baseUrls), list(articleTitle),
                  list(articleDate), list(articleBody), list(lang), list(avoid))
    save = False
    syncState = False
    for prname in projectNames:
        ...
        for filename in os.listdir(prname+"/htmlFiles"):
            ...
            plainText, tmpScore, title, article_date = prep.parseDocument(
                "main", prname, openedFile)
            # TODO article_date to used in ranking
            if syncState:
            ...
            if plainText is not None:
                ...
                print(">>>>>>>>>>>>>>>>", plainText)
                save = indexer.addDocument(
                    "main", prname, base64.b16decode(filename),
                    workaround.Document(plainText, tmpScore, title))
        if save:
            print("+++", "saving...")
